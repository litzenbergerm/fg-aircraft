...
var DME_format = func (x) { return sprintf((x>10) ? "%3.0f" : "%4.1f", x); }; 
...
#return distance of selected nav source in nm

var navSrcDist = func (x) {
  if (x == "NAV1")
     return adc.nav0dist.val * DMEFACTOR;
  if (x == "NAV2")
     return adc.nav1dist.val * DMEFACTOR;
  if (x == "ADF1")
     return "";
  if (x == "FMS")
     return adc.fmsdist.val;
  return ""; 
};
...
var generateAltLadder = func (p, pitch=46.5, post500=1, post100=0) {
    var _id = "";
    ...
    for (var i=1;  i<200; i+=1) {
      ...
      p["Hnum0"~_id].setText("" ~ math.mod(i,10) ~ ( (math.mod(i,5)==0 and post500) or (math.mod(i,1)==0 and post100) ? "00" : "" ));
